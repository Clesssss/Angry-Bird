...
function main() {
    ...
    var animate = function (time) {
        ...
        LIBS.translateX(MODEL_MATRIX4, - 3);
        ...
        LIBS.translateX(MODEL_MATRIX, 0 + offsetSplineX + offsetSplineXreverse);
        LIBS.translateY(MODEL_MATRIX, 4);

        LIBS.rotateY(MODEL_MATRIX, - Math.PI / 2);

        if (time > 10000 && offsetSplineX > -18) {
            let rotation = time / 400;
            LIBS.rotateZ(MODEL_MATRIX, rotation)
        ...
        if (backwardsCondition) {
            let rotation = time / 400;
            LIBS.rotateZ(MODEL_MATRIX, -rotation + 1.62)
